...
class UsersImport implements OnEachRow, SkipsOnError, WithHeadingRow, WithStartRow
{
    ...
    public function onRow(Row $row)
    {
        ...
        try {
            ...
            $flat = DB::table('flat')->join('apartment', 'flat.apartment_id', '=', 'apartment.id')->where('flat.name', $row['suite'])->where('apartment.id', 2)->first('flat.id');
            if (null  === $flat) {
                $this->onError(ValidationException::withMessages(['email' => "Couldn't create user for {$row['suite']}: Flat Didn't exists"]));
                return;
            }

            if (User::where('first_name', $row['first_name'])->where('last_name', $row['lastname'])->exists()) {
                $user = User::where('first_name', $row['first_name'])->where('last_name', $row['lastname'])->get()->first();
            } else {
            ...
            if (!UserProfile::where('user_id', $user->id)->exists()) {
                ...
                'total_rent'  => str_replace([' ', '$', '-'], 0,$row['total_rent'] ?? 0),
                ...
                'wifi' => str_replace([' ', '$'], '',empty(trim($row['wifi_fee']?? 0)) ? 0 :(float)$row['wifi_fee']),
                ...
                'addtl_deposits' => $row['addtl_deposits'] ?? null,
                'fob_id_2' => $row['fob_id_2'] ?? null,
                'will' => $row['will'] ?? null,
                'ice_name' => $row['ice_name'] ?? null,
                'emergency_address' => $row['address'] ?? null,
                'family_doctor' => $row['family_doctor'] ?? null,
